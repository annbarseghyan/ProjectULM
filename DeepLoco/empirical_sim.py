...
class EmpiricalSim(object):
    ...
    def __init__(self, n_pixels_frame, frame_width_nm, stack_data, multiplier=1.0/250000):#STACK_NPZ_FILENAME):
        ...
        """
        # noise mean reduced from 100 to 0
        noise_mean = 0.0 # ? % ? % ? %
        self.multiplier = multiplier#1/100000.0
        # reduced psf_with_nm from 400 to 150
        self.psf_width_nm = 25
    ...
    def draw(self, x,y,
        ...
        # reduced max_dist from 2700 to 1000
        max_dist = 200.0
        ...
        spline = self.splines[75]  # Assuming z=0 is at index 75 (midpoint of 150 slices)

        # for _ in range(6):  # Ensure 5-6 bubbles
        #     p_idx = np.random.randint(4)
        #     (xs, ys) = self.offsets[75]
        #     x_s, y_s = xs[p_idx], ys[p_idx]
        #
        #     f_y = (new_pixel_locations + (y_s - y))[y_filter]
        #     f_x = (new_pixel_locations + (x_s - x))[x_filter]
        #
        #     d_image = spline(f_y, f_x)
        #     image[y_l:y_u, x_l:x_u] += self.multiplier * w * d_image


        p_idx = np.random.randint(4)  # Random offset index, as in original
        (xs, ys) = self.offsets[75]  # Corresponding offsets for z=0
        x_s, y_s = xs[p_idx], ys[p_idx]

        f_y = (new_pixel_locations + (y_s - y))[y_filter]
        f_x = (new_pixel_locations + (x_s - x))[x_filter]

        d_image = spline(f_y, f_x)  # Evaluate spline at 2D coordinates
        image[y_l:y_u, x_l:x_u] += self.multiplier * w * d_image  # Scale by weight and multiplier
...
#TODO: WE ARE HARDCODING 1 PIXEL = 100 NM.
    def run_model(self, thetas, weights):
        ...
        assert thetas.shape == (batch_size, MAX_N, 2)
        ...
        for b_idx in range(batch_size):
            ...
            for (w, (x,y)) in zip(weights[b_idx], thetas[b_idx]):
                if w != 0.0:
                    self.draw(x, y, image, w)
                # for _ in range(5):  # Increase bubble density
                #     x_offset = np.random.uniform(-200, 200)
                #     y_offset = np.random.uniform(-200, 200)
                #     self.draw(x + x_offset, y + y_offset, image, w)
