...
def munkres(costMat):
    ...
    while True:
        ...
        rIdxcIdx = np.argwhere(
            dMat[~coverRow[:, 0]][:, ~coverColumn] == minR[~coverRow][:, np.newaxis] + minC[~coverColumn])
        ...
        while True:
            ...
            if Step == 6:
                minval, rIdx, cIdx = outerplus(dMat[~coverRow[:, 0]][:, ~coverColumn], minR[~coverRow],
                                               minC[~coverColumn])
...
def generate_random_points_uniform(n_points=1, nb_frames=10, max_speed=0.2):
    ...
    for p in range(n_points):
        ...
        for f in range(1, nb_frames):
            point = (f, round(tracks[p][-1][1] + random.uniform(-max_speed, max_speed), 3),
                     round(tracks[p][-1][2] + random.uniform(-max_speed, max_speed), 3))
            tracks[p].append(point)
    return tracks
...
def generate_random_points_directional(n_points=1, nb_frames=10, speed_variance=0.2, angular_variance=2 * math.pi / 10):
    ...
    for p in range(n_points):
        ...
        for f in range(1, nb_frames):
            ...
            point = (f, round(tracks[p][-1][1] + math.cos(current_angle) * current_speed, 3),
                     round(tracks[p][-1][2] + math.sin(current_angle) * current_speed, 3))
            tracks[p].append(point)
    return tracks


def generate_tracking_example(n_points=1, nb_frames=10, probability_disapear=0,
                              function=generate_random_points_uniform):
    ...
    for track_ind, current_tracking in enumerate([tracking, rand_original_cut, original]):
        for track in current_tracking:
            ...
            for i in range(len(track)):
                axs[track_ind].plot(track[i][1], track[i][2], marker='o',
                                    color=(1, track[i][0] / nb_frames, track[i][0] / nb_frames))
...
def tracking(slices, max_linking_distance, max_gap_closing, min_length, time_per_frame, res):
    ...
    for i_track, track in enumerate(simple_tracks):
        ...
        zu = interp1d(np.arange(zi.shape[0]), track_interp_smooth(zi, smooth_factor))(
            np.around(np.arange(0, zi.shape[0] - 1 + 1e-10, interp_factor), 10))
        xu = interp1d(np.arange(xi.shape[0]), track_interp_smooth(xi, smooth_factor))(
            np.around(np.arange(0, xi.shape[0] - 1 + 1e-10, interp_factor), 10))
        TimeAbs_interp = interp1d(np.arange(timeAbs.shape[0]), track_interp_smooth(timeAbs, smooth_factor))(
            np.around(np.arange(0, timeAbs.shape[0] - 1 + 1e-10, interp_factor), 10))
        # print(xu)
        # print(zu)
...
def show_tracking_simple(tracks, plot_save_path):
    """
    Displays tracking data as lines without enumerating individual points.

    Args:
        tracks (list): List of tracked points in the form [(frame_idx, x, y)].
        nb_frames (int): Number of frames (for color normalization).
    """
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.set_title("Tracked Motion")
    ax.set_xlabel("X Position")
    ax.set_ylabel("Y Position")

    for track in tracks:
        r, g = random.uniform(0.25, 0.75), random.uniform(0.25, 0.75)
        color = (r, g, 1 - (g + r) / 2)  # Unique color for each track

        for i in range(len(track) - 1):
            x_vals = [track[i][1], track[i + 1][1]]
            y_vals = [track[i][2], track[i + 1][2]]

            if track[i + 1][0] > track[i][0] + 1:
                ax.plot(x_vals, y_vals, color=color, linestyle='--')  # Dashed line for gaps
            else:
                ax.plot(x_vals, y_vals, color=color)  # Solid line for continuous tracking
